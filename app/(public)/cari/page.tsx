          ...
          <Col xs={12} sm>
            <Stack gap={2}>
              <div className="article-meta">
                {category} <span className="text-muted mx-1">•</span>{" "}
                {article.publishedAt}
              </div>
  ...
  const renderPagination = () => {
      ...
      if (totalPages <= 5) {
        for (let p = 1; p <= totalPages; p++) {
          buttons.push(
            <button
              key={p}
              className={`pagination-button pagination-number-button ${
    ...
    return (
      <nav className="pagination-nav">
        ...
        >
          <span className="material-symbols-outlined pagination-arrow">
            arrow_back
          </span>
          Prev
        </Button>
