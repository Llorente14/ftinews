...
function slugToCategoryName(slug: string, categories: { name: string; count: number }[]): string | null {
        ...
        <nav className={`${styles.nav} ${isMobileMenuOpen ? styles.navOpen : ""}`}>
          <div className={styles.navContent}>
            <div className={styles.navLinks}>
              ...
              {/* Menu News aktif */}
              <Link href="#" className={styles.navLinkActive}>News</Link>
              {session && <Link href="/profile" className={styles.navLink}>Profile</Link>}
      ...
      <main className={styles.main}>
        ...
        <section className={styles.categoryHeader}>
          ...
          <p className={styles.categoryDesc}>
            Berita terkini dan terpercaya seputar {categoryName || (slug ? slug.replace(/-/g, " ") : "...")}.
          </p>
          ...
          <div className={styles.filters}>
            <div className={styles.filterGroup}>
              ...
              <div className={styles.dropdownContainer}>
                <button
                  className={`${styles.filterBtn} ${sortType === "newest" || sortType === "oldest" ? styles.filterBtnActive : ""}`}
                  onClick={() => toggleDropdown("date")}
                >
                  Date: {sortType === "newest" ? "Latest" : sortType === "oldest" ? "Oldest" : "Latest"}
                  <i className={`bx ${openDropdown === "date" ? "bx-chevron-up" : "bx-chevron-down"}`} style={{ fontSize: '18px', marginLeft: '6px', verticalAlign: 'middle' }}></i>
                </button>
                {openDropdown === "date" && (
                  <div className={styles.dropdownMenu}>
                    <button
                      className={`${styles.dropdownItem} ${sortType === "newest" ? styles.dropdownItemActive : ""}`}
                      onClick={() => handleSortChange("newest")}
                    >
                      Latest
                    </button>
                    <button
                      className={`${styles.dropdownItem} ${sortType === "oldest" ? styles.dropdownItemActive : ""}`}
              ...
              <div className={styles.dropdownContainer}>
                <button
                  className={`${styles.filterBtn} ${selectedAuthorId ? styles.filterBtnActive : ""}`}
                  onClick={() => toggleDropdown("author")}
                >
                  <span>
                    Author: {selectedAuthorId 
                      ? authors.find(a => a.id === selectedAuthorId)?.namaLengkap || "All Authors"
                      : "All Authors"}
                  </span>
                  <i className={`bx ${openDropdown === "author" ? "bx-chevron-up" : "bx-chevron-down"}`} style={{ fontSize: '18px', marginLeft: '6px', verticalAlign: 'middle' }}></i>
                </button>
                {openDropdown === "author" && (
                  <div className={styles.dropdownMenu}>
                    <button
                      className={`${styles.dropdownItem} ${!selectedAuthorId ? styles.dropdownItemActive : ""}`}
                    ...
                    {authors.length > 0 ? (
                      authors.map((author) => (
                        <button
                          key={author.id}
                          className={`${styles.dropdownItem} ${selectedAuthorId === author.id ? styles.dropdownItemActive : ""}`}
                      ...
                      <div className={styles.dropdownItem} style={{ opacity: 0.6 }}>
                        No authors found
                      </div>
                    )}
                  </div>
            ...
            <div className={styles.dropdownContainer}>
              <button
                className={`${styles.filterBtn} ${sortType === "az" || sortType === "za" ? styles.filterBtnActive : ""}`}
                onClick={() => toggleDropdown("sort")}
              >
                Sort by: {sortType === "az" ? "A-Z" : sortType === "za" ? "Z-A" : "A-Z"}
                <i className={`bx ${openDropdown === "sort" ? "bx-chevron-up" : "bx-chevron-down"}`} style={{ fontSize: '18px', marginLeft: '6px', verticalAlign: 'middle' }}></i>
              </button>
              {openDropdown === "sort" && (
                <div className={styles.dropdownMenu}>
                  <button
                    className={`${styles.dropdownItem} ${sortType === "az" ? styles.dropdownItemActive : ""}`}
                    onClick={() => handleSortChange("az")}
                  >
                    A-Z
                  </button>
                  <button
                    className={`${styles.dropdownItem} ${sortType === "za" ? styles.dropdownItemActive : ""}`}
                ...
                >
                  ...
                  <div className={styles.cardContent}>
                    ...
                    <div className={styles.cardFooter}>
                      ...
                      <i className='bx bx-bookmark' style={{ fontSize: '18px' }}></i>
                    </div>
                  </div>
                </Link>
              ))
            ) : (
              <div className={styles.emptyState}>Belum ada berita di kategori ini.</div>
            )}
          </section>
        )}
