...
import { useBookmarks, useDeleteBookmark, Bookmark } from '@/hooks/useBookmarks';
...
const BookmarkPage: React.FC = () => {
  ...
  useEffect(() => {
    if (error === "Silakan login terlebih dahulu." || (!session && !isLoading && !error)) {
        router.push('/login');
    }
      ...
      return (
          <div className={styles.modalBackdrop}>
              <div className={styles.modalContent}>
                  <i className='bx bx-trash' style={{ fontSize: '2rem', color: '#ef4444' }}></i>
                  <h3 className={styles.modalTitle}>Hapus Artikel?</h3>
                  <p className={styles.modalText}>
                      Apakah Anda yakin ingin menghapus artikel ini dari koleksi Anda? Tindakan ini tidak dapat dibatalkan.
                  </p>
  ...
  if (error && error !== "Silakan login terlebih dahulu.") {
    return (
      <div className={styles.centerState}>
        <i className='bx bx-error-alt' style={{ fontSize: '2.5rem', color: '#ef4444' }}></i>
        <p>Error: {error}</p>
      </div>
    );
  }
        ...
        <div className={styles.grid}>
          {filteredAndSortedBookmarks.length > 0 ? (
            filteredAndSortedBookmarks.map((bookmark: Bookmark) => (
              <div key={bookmark.id} className={styles.card}>
                <div
                    ...
                    style={{ 
                      backgroundImage: `url(${bookmark.article?.imageUrl || '/placeholder.jpg'})` 
                    }}
                />
                ...
                <div className={styles.cardContent}>
                  ...
                  <p className={styles.cardDesc}>
                    {bookmark.article?.description || 'Deskripsi tidak tersedia'}
                  </p>
            ...
            <div className={styles.emptyState}>
              <i className='bx bx-bookmark' style={{ fontSize: '3rem', opacity: 0.5 }}></i>
              <p>
